...
class MapVisualizer
{
    ...
    */
    ...
    public MapVisualizer(GameObject hexObject_, 
    ...
    public delegate void offsetFunc(int w, int h, ref Vector3 pos);

    public void oddRowOffset(int w, int h, ref Vector3 pos)
    {
        float offset = h % 2 == 0 ? 0 : wi / 2;
        pos += new Vector3(offset, 0, 0);
    }

    public void evenRowOffset(int w, int h, ref Vector3 pos)
    {
        float offset = h % 2 == 1 ? 0 : wi / 2;
        pos += new Vector3(offset, 0, 0);
    }

    public void oddColumnOffset(int w, int h, ref Vector3 pos)
    {
        float offset = w % 2 == 0 ? 0 : hi / 2;
        pos += new Vector3(0, 0, offset);
    }

    public void evenColumnffset(int w, int h, ref Vector3 pos)
    {
        float offset = w % 2 == 0 ? 0 : hi / 2;
        pos += new Vector3(0, 0, offset);
    }

    public void setupSquareGrids(Vector3 mapCenter, Vector3 mapSize, float squareSize)
    {
        wi = squareSize;
        hi = squareSize;
        ...
        int horizontalSquareNum = (int)Mathf.Ceil(mapSize.z / hi);
        int verticalSquareNum = (int)Mathf.Ceil(mapSize.x / wi);

        //OffsetFunctions = new List<offsetFunc>() { oddRowOffset, evenRowOffset, evenColumnffset, oddColumnOffset };
        Vector3 origin = mapCenter - mapSize / 2 + new Vector3(squareSize / 2.0f, 0.0f, squareSize / 2.0f);
        constructSquareGrid(verticalSquareNum, horizontalSquareNum, origin);
    }

    public void spawnNode(MapNode node)
    {
        Vector3 pos = node.position;
        GameObject tile = GameObject.Instantiate(squareObject, pos, Quaternion.identity);
        tile.transform.localScale = this.cubeScale;
        tile.transform.localScale *= 0.9f;

        Color tile_color = node.color;
        tile.GetComponent<Renderer>().material.SetColor("_Color", tile_color);
        tile.GetComponent<Renderer>().material.shader = Shader.Find("Diffuse");
        tile.transform.parent = map.transform;
    }


    public void setupGridParams(int type)
    {
        //if (type == 0)
        //{
    ...
    public void constructHexGrid(int width, int height, Vector3 origin, offsetFunc offset_func)
    {
    ...
    public void constructSquareGrid(int width, int height, Vector3 origin)
    {
        ...
        for (int h = 0; h < height; h++)
        {
            for (int w = 0; w < width; w++)
            {
                Vector3 pos = origin + new Vector3(w * wi, heightMod * Random.value, h * hi);
                GameObject tile = GameObject.Instantiate(squareObject, pos, Quaternion.identity);
    ...
    public void styleObjectFromGrid(GameObject go, int h, int w)
    {
